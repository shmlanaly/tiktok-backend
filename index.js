...
app.get('/make-viral-video', async (req, res) => {
  // السيرفر سيقرأ المفتاح من إعدادات Railway
  const API_KEY = process.env.GROQ_API_KEY;
  const url = "https://api.groq.com/openai/v1/chat/completions";

  try {
    const response = await axios.post(url, {
      model: "llama3-8b-8192", // نموذج سريع ومجاني تماماً
      messages: [
        {
          role: "user",
          content: "اكتب قصة رعب يمنية قصيرة جداً ومشوقة بلهجة صنعانية، تنتهي بنهاية صادمة جداً لتيك توك."
        }
      ]
    }, {
      headers: {
    ...
    res.send(`
      <div style="font-family:sans-serif; text-align:center; padding:20px; background:#000; color:#fff; min-height:100vh;">
        <h1 style="color:#00f2ea;">🚀 متصل بـ Groq (البديل المجاني)</h1>
    ...
    res.status(500).send("خطأ Groq: " + (err.response ? JSON.stringify(err.response.data) : err.message));
  }
});

app.listen(port, '0.0.0.0', () => console.log('Groq Bot is Live!'));
